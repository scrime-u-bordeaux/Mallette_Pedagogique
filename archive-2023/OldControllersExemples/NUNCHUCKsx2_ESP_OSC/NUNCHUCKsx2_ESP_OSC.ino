...
char ssid[] = "dlink"; //"NUMERICABLE-A7A6";          // your network SSID (name)
char pass[] = "espace29";                    // your network password
...
void parse() {
  ...
  bndl.add("/blackNun/Xaccel").add((float)accel_x_axis / 1023);
  bndl.add("/blackNun/Yaccel").add((float)accel_y_axis / 1023);
...
#endif
  OSCBundle bndl; //BOSCBundle's add' returns the OSCMessage so the message's 'add' can be composed together
  bndl.add("/whiteNun/Xaccel").add((float)accel_x_axis / 1023);
  bndl.add("/whiteNun/Yaccel").add((float)accel_y_axis / 1023);
  //bndl.add("/whiteNun/Zaccel").add((float)accel_z_axis / 1023);
  bndl.add("/whiteNun/Xjoy").add((float)joy_x_axis / 255);
  bndl.add("/whiteNun/Yjoy").add((float)joy_y_axis / 255);
  bndl.add("/whiteNun/Cbutt").add((float)c_button);
  bndl.add("/whiteNun/Zbutt").add((float)z_button);
  Udp.beginPacket(outIp, outPort);
  bndl.send(Udp);
  Udp.endPacket();
  bndl.empty();
